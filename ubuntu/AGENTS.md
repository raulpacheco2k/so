...
- O menu Wi-Fi (`wifi-menu.sh`) usa `nmcli` como backend e com o rádio desligado mostra apenas a opção de ativá-lo. Ligado, oferece desativar, desconectar da rede atual, um submenu `Gerenciar: <nome>` por rede salva e `Conectar nova rede Wi-Fi`; o menu principal não lista redes visíveis. O submenu de rede salva tem conectar/desconectar, `Esquecer rede` e `Voltar`; esquecer exige confirmação, desconecta antes de remover o perfil e só confirma sucesso depois que o perfil deixa de existir no NetworkManager. `Conectar nova rede Wi-Fi` abre uma descoberta em tempo real das redes próximas não salvas (recriando o menu quando a lista muda) e pede a senha pelo próprio dmenu, sem abrir aplicações externas; um perfil criado com senha errada é removido após a falha. Os prompts usam breadcrumbs (`Wi-Fi > <rede>`, `Wi-Fi > <rede> > Esquecer?` e `Wi-Fi > Conectar nova rede`), as ações confirmam com notificação e redes novas pedem a senha no próprio dmenu quando não há segredos armazenados.
