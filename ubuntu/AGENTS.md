...
- O menu Bluetooth (`bluetooth-menu.sh`) nunca abre terminal: conectar não pareia, o pareamento solicita PIN/passkey/autorização por dmenu ou Walker e vínculos rejeitados são reparados somente por uma ação explícita. Após conectar, o sink Bluetooth reportado pelo servidor de áudio é definido como padrão quando disponível.
